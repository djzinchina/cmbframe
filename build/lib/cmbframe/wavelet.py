...
#########################################################################
"""
CMBframe wavelet transformation module provides functions and utilities
...
def mexican_bands(nbands, lmax_band):
    B = 1.5
    mexi_bands = np.zeros((lmax_band+1, nbands))
    ells = np.arange(lmax_band+1, dtype=np.float64)
    for band in range(nbands):
        j = band + 1
        mexi_bands[:,band] = ells/(B**j)/np.sqrt(2.*np.pi)*np.exp(-(ells/(B**j))**2/2)
...
def get_lmax_band(band):
    ...
    if lmax_band != lmax or band[lmax_band] < 5.e-2:
        wavelet_peak = np.min(np.where(band == np.max(band))[0])
        lmax_band = wavelet_peak - 1 + min(np.where(band[wavelet_peak:] < 5.e-2)[0])

    return lmax_band

def alm2wavelet(alm_in, bands, nodegrade=False, wavelet_nside=None, w_nside_min=None, w_nside_max=None):
    """
    Performs wavelet transform from input spherical harmonic coefficient (alm) of a map.
    ...
    nodegrade : bool, optional
        Sets if all wavelet scale maps have the same nside.
    ...
    wavelet_nside : int, optional
        Nside of wavelet maps at all bands. Default is None.
    w_nside_min : int, optional
        Minimum nside for any wavelet scale maps. Default is None.
    w_nside_max : int, optional
        Maximum nside for any wavelet scale maps. Default is None.

    Notes
    -----
    When nside is not set by user the nside of each wavelet scale map 
    ...
    if np.array(alm_in).ndim != 1:
        print("ERROR: Cannot wavelet transform multiple maps.")
        exit()
    ...
    # if lmax > lmax_sht:
    #     wavelet_bands = np.copy(bands[:lmax_sht+1,:nbands]) 
    #     print("Note bands have higher lmax than alms. Some wavelets may have no information")
    # else :
    wavelet_bands = np.copy(bands)
    ...
    for band in range(nbands):
        ...
        lmax_band = get_lmax_band(wavelet_bands[:, band])

        # # lmax_band = np.max(np.where(wavelet_bands[:,band] > 0))
        # lmax_band = np.max(np.where(wavelet_bands[:,band] > 0)[0])
        # # print(lmax_band)
        # if lmax_band != lmax or wavelet_bands[lmax_band,band] < 5.e-2:
        #     wavelet_peak = np.min(np.where(wavelet_bands[:,band] == np.max(wavelet_bands[:,band]))[0])
        #     lmax_band = min(np.where(wavelet_bands[wavelet_peak:,band] < 5.e-2)[0])
        
        if not nodegrade:
            nside_band = np.min(nside_opts[nside_opts >= lmax_band])
            if w_nside_min != None :
                if nside_band < w_nside_min : nside_band = w_nside_min
            if w_nside_max != None :
                if nside_band > w_nside_max : nside_band = w_nside_max 
        else :
            if wavelet_nside != None:
                nside = wavelet_nside
            else:
                nside = 2**(mt.ceil(mt.log2(lmax)))

            nside_band = nside 

        # print(nside_band, lmax_band)

        wavalm = hp.almxfl(np.copy(alm_in),wavelet_bands[:,band])
        wavelet_maps.append(hp.alm2map(wavalm, nside_band, pol=False, verbose=False))
...
def map2wavelet(map_in, bands, wavelet_nside=None, nodegrade=False, w_nside_min=None, w_nside_max=None):
    """
    Performs wavelet transform from input healpix map. This
    is a map level wrapper for alm2wavelet function.
    ...
    if lmax_wv > lmax_map:
        print("WARNING: lmax of wavelet bands exceeds map lmax limit.")
        lmax_sht = lmax_map
    else:
        lmax_sht = lmax_wv

    alm_temp = hp.map2alm(map_in, lmax=lmax_sht, pol=False, use_weights=True, datapath='/home/doujzh/Healpix_3.50/data')

    wavelet_map = alm2wavelet(alm_temp, bands, wavelet_nside=wavelet_nside, nodegrade=nodegrade, w_nside_min=w_nside_min, w_nside_max=w_nside_max)
...
def wavelet2alm(wvlt_map_in, bands):
    """
    Converts wavelet maps back to spherical harmonic coefficients (alm).
    ...
    for band in range(nbands):
        ...
        # print(wvlt_map_in[band].shape, band)
        alm_band = hp.map2alm(np.copy(wvlt_map_in[band]),lmax=lmax_band, pol=False, use_weights=True, datapath="/home/doujzh/Healpix_3.50/data")
...
def wavelet2map(nside, wvlt_map_in, bands, map_outfile=None):
    """
    Converts wavelet maps back to single healpix map combining all needlet scales.
    This is a wrapper for wavelet2alm function.
    ...
    alm_temp = wavelet2alm(wvlt_map_in, bands)
...
def write_waveletmap_fits(bands, wvlt_map_in, outfile, unit=''):
    """
    Writes wavelet maps and corresponding needlet band information to a fits file.
    ...
    Notes
    -----
    Saves the needlet bands in the PrimaryHDU of the fits file. Saves the wavelet maps
    ...
    for i in range(len(bands[0,:])):
        tbhdu = fits.BinTableHDU.from_columns([fits.Column(name='Band '+str(i), unit=unit, format='D', array=wvlt_map_in[i])])
...
def read_waveletmap_fits(infile, band_no=None):
    """
    Reads wavelet map(s) contained in the BintableHDU of a fits file created with
    ...
    with fits.open(infile) as hdulist:
        wavelets = np.array(hdulist[0].data)

        nbands = len(wavelets[0,:])
...
def read_waveletbands_fits(infile):
    """
    ...
    numpy ndarray
        A 2D numpy array of shape ``(lmax+1, nbands)`` conatining needlet bands
        used to produce the wavelet scale maps contained in the file.
    """
