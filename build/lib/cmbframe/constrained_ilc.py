...
class cilc_cleaner:
    ...
    def compute_cilc_weights(self, map_sel, constr_comp=[1,2], bandpass='delta', lmax_ch=None):
        """
        ...
        for nu_1 in range(0,self.nu_dim) :
            for nu_2 in range(nu_1,self.nu_dim) :
                if nu_2 == nu_1:
                    har_cov_ij[:,nu_1,nu_1] = cu.calc_binned_cov(self.alms[nu_1])[2:]
                else:
                    har_cov_ij[:,nu_1,nu_2] = har_cov_ij[:,nu_2,nu_1] = cu.calc_binned_cov(self.alms[nu_1], alm2=self.alms[nu_2])[2:]
