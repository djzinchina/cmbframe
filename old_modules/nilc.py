...
__pdoc__ = {}
__pdoc__['nilc_weights2.ugrade_wgt'] = False

input_map_prefix = '/map_in_wav-transform_nu'
proj_map_prefix = '/comp_in_wav-transform_nu'
...
class nilc_workspace:
        ...
        for nu in tqdm(range(self.nu_dim), ncols=120):
            ...
            if isIQU == 1:
                ...
                if nside_nu != nside_mask:
                    ...
                    mask_nu = self.mask
                alm_nu = hp.map2alm(maps_in[nu]*mask_nu, lmax=self.lmax_sht, pol=False, use_pixel_weights=True, datapath='/home/shamik/DATA/HPX_pix_wgts')
            elif isIQU == 2:
                ...
                if nside_nu != nside_mask:
                    ...
                    mask_nu = self.mask
                alm_nu = hp.map2alm(maps_in[nu]*mask_nu, lmax=self.lmax_sht, pol=False, use_pixel_weights=True, datapath='/home/shamik/DATA/HPX_pix_wgts')[TEB]
            ...
            if self.low_memory:
                wv.write_waveletmap_fits(self.bands, wav_maps_nu, outfile=self.scratch+input_map_prefix+str(nu)+'.fits')
            else:
                if nu == 0:
    ...
    def ugrade_wgt(self, weights, nside_up):
        nside_low = hp.npix2nside(len(weights[:,0,0]))
        n_valid_ch = len(weights[0,:,0])
        s2nind = su.npix2spix_map(nside_up, nside_low)

        nilc_wgts_mapped = np.zeros((hp.nside2npix(nside_up), n_valid_ch))
        nilc_wgts_mapped = weights[s2nind,:,0]
    ...
    def compute_nilc_weights(self, e_vec=None, lmax_ch=None, cov_method='dgrade', ilc_bias=0.03, return_clean_map=True):
        ...
        if isinstance(e_vec, (list, np.array)):
            if len(e_vec) != self.nu_dim:
                print("ERROR: e_vec length does not match number of maps.")
        else:
            e_vec = np.ones((self.nu_dim,), dtype=np.float64)

        if np.logical_not(isinstance(lmax_ch, (list, np.array))):
            self.lmax_ch = self.lmax * np.ones_like(np.array(nu_arr))
        else:
        ...
        for band in tqdm(range(self.nbands), ncols=120):
            ...
            for nu in nu_arr[self.lmax_ch >= lmax_band]:
                if self.low_memory:
                    map_nus_in_band.append(wv.read_waveletmap_fits(self.scratch+input_map_prefix+str(nu)+'.fits', band_no=band))
                else:
            ...
            if self.low_memory:
                np.save(self.scratch+'/nilc_weights_band'+str(band)+'.npy', weights)
            else:
                if band == 0:
                    self.weights_by_band = weights
                else:
                    self.weights_by_band.append(weights)
            
            if return_clean_map:
                wgts_by_nu = self.ugrade_wgt(weights, hp.npix2nside(len(map_nus_in_band[0])))
    ...
    def get_nilc_maps(self):
        if self.weights_done :
            ...
            for band in range(self.nbands):
                ...
                if self.low_memory:
                    weights = np.load(self.scratch+'/nilc_weights_band'+str(band)+'.npy')
                else:
    ...
    def get_projected_map(self, maps_in, TEB=0, adjust_beam=True):
        ...
        for nu in tqdm(range(self.nu_dim), ncols=120, leave=False):
            nside_mask = hp.npix2nside(len(self.mask))
            if isIQU == 1:
                nside_nu = hp.npix2nside(len(maps_in[nu]))
                if nside_nu != nside_mask:
            ...
            if self.low_memory:
                wv.write_waveletmap_fits(self.bands, wav_maps_nu, outfile=self.scratch+proj_map_prefix+str(nu)+'.fits')
        ...
        if self.weights_done :
            ...
            for band in range(self.nbands):
                ...
                for nu in nu_arr[self.lmax_ch >= lmax_band]:
                    if self.low_memory:
                        map_nus_in_band.append(wv.read_waveletmap_fits(self.scratch+proj_map_prefix+str(nu)+'.fits', band_no=band))
                    else:
